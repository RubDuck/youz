<template>
 <div class="container " style="min-height: 597px;">
        ...
        <div class="block-item">
          ...
          <div class="select-group">
            <select class="js-province-selector" v-model='provinceValue' >
              ...
              <option  :value='list.value' v-for='list in citydata.list'>{{list.label}}</option>
            </select>
            <select class="js-city-selector"  v-model='cityValue'>
              <option value="-1">选择城市</option>
              <option  :value='list.value' v-for='list in cityName'>{{list.label}}</option>
            </select>
            <select class="js-county-selector" name="area_code" data-code="" v-model='districtValue'> 
              <option value="-1">选择地区</option>
               <option  :value='list.value' v-for='list in Districtname'>{{list.label}}</option>
            </select>
          </div>
        </div>
        <div class="block-item">
    ...
    <div class="block section js-delete block-control-btn" v-show="status=='modify'" @click='deldate'>
      <div class="block-item c-red center">删除</div>
    </div>
    <div class="block stick-bottom-row center js-save-default " v-show="status=='modify'" @click='setDefault'>
